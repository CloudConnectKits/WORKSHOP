 ...
 * readings are published at 5 second intervals. All readings are
 ...
 *
 * NOTE:
 *   The ZAP may not work with a debug build if TLS is enabled on platforms
 ...
 *                    Structures
 ******************************************************/
const accelerometer_config_t const accel_config =
{
    .fullscale = ACCEL_FULLSCALE_2G,
    .samp_freq = ACCEL_SAMP_FREQ_50HZ,
    .axis_en = ACCEL_AXIS_EN_X | ACCEL_AXIS_EN_Y | ACCEL_AXIS_EN_Z,
    .mode = ACCEL_MODE_NORMAL
};

const gyroscope_config_t const gyro_config =
{
    .fullscale = GYRO_FULLSCALE_250DPS,
    .axis_en = GYRO_AXIS_EN_X | GYRO_AXIS_EN_Y | GYRO_AXIS_EN_Z,
};
...
void zn_app_init(void)
{
    ...
    ZOS_LOG("\r\n");
    if (sensor_init(SENSOR_ACCELEROMETER, (void*)&accel_config) != ZOS_SUCCESS)
    {
        ZOS_LOG("ERROR - Failed to initialize accelerometer!");
    }
    else
    {
        ZOS_LOG("Accelerometer initialized successfully!");
    }

    if (sensor_init(SENSOR_MAGNETOMETER, (void*)NULL) != ZOS_SUCCESS)
    {
        ZOS_LOG("ERROR - Failed to initialize magnetometer!");
    }
    else
    {
        ZOS_LOG("Magnetometer initialized successfully!");
    }

    if (sensor_init(SENSOR_GYROSCOPE, (void*)&gyro_config) != ZOS_SUCCESS)
    {
        ZOS_LOG("ERROR - Failed to initialize gyroscope!");
    }
    else
    {
        ZOS_LOG("Gyroscope Initialized successfully!");
    }
    ...
    ZOS_LOG("  - Topic/Queue : %s",     settings.topic);
    ZOS_LOG("  - Client ID   : %s\r\n", settings.client);

    ZOS_LOG("Website  : https://quickstart.internetofthings.ibmcloud.com");
    ZOS_LOG("Device ID: "DEVICE_ID"\r\n", mac_str[12], mac_str[13], mac_str[15], mac_str[16]);
...
static zos_result_t mqtt_connection_event_cb( mqtt_event_info_t *event )
{
    switch ( event->type )
    {
        case MQTT_EVENT_TYPE_CONNECTED:
            ...
            ZOS_LOG("Publishing sensor readings periodically to IBM Bluemix broker..." );

            /* once connected, trigger event to publish a message */
            zn_event_issue(periodic_publish, NULL, 0);
            //zn_event_register_periodic(periodic_publish, NULL, MQTT_PUBLISH_PERIOD, EVENT_FLAGS1(RUN_NOW));
            break;
...
/*************************************************************************************************
 * Periodic publish sensor readings
 */
static void periodic_publish(void *arg)
{
    zos_bool_t has_data;
    // static objects, so use old values if not new data reported by sensors
    static accelerometer_data_t accl_data = {0};
    static magnetometer_data_t magn_data = {0};
    static gyroscope_data_t gyro_data = {0};

    // Accelerometer readings:
    if (sensor_has_new_data(SENSOR_ACCELEROMETER, &has_data) == ZOS_SUCCESS)
    {
        if (has_data == ZOS_TRUE)
        {
            if (sensor_get_data(SENSOR_ACCELEROMETER, &accl_data) == ZOS_SUCCESS)
            {
                //ZOS_LOG("Accelerometer (mG): %d, %d, %d", accl_data.x, accl_data.y, accl_data.z);
            }
            else
            {
                ZOS_LOG("Accelerometer: Failed to get data!");
            }
        }
    }
    else
    {
        ZOS_LOG("Accelerometer: Failed to check for new data!");
    }

    // Magnetometer readings:
    if (sensor_has_new_data(SENSOR_MAGNETOMETER, &has_data) == ZOS_SUCCESS)
    {
        if (has_data == ZOS_TRUE)
        {
            if (sensor_get_data(SENSOR_MAGNETOMETER, &magn_data) == ZOS_SUCCESS)
            {
                //ZOS_LOG("Magnetometer (uT): %d, %d, %d", magn_data.x, magn_data.y, magn_data.z);
            }
            else
            {
                ZOS_LOG("Magnetometer: Failed to get data!");
            }
        }
    }
    else
    {
        ZOS_LOG("Magnetometer: Failed to check for new data!");
    }

    // Gyroscope readings:
    if (sensor_has_new_data(SENSOR_GYROSCOPE, &has_data) == ZOS_SUCCESS)
    {
        if (has_data == ZOS_TRUE)
        {
            if (sensor_get_data(SENSOR_GYROSCOPE, &gyro_data) == ZOS_SUCCESS)
            {
                //ZOS_LOG("Gyroscope (deg/sec): %d, %d, %d", gyro_data.x, gyro_data.y, gyro_data.z);
            }
            else
            {
                ZOS_LOG("Gyroscope: Failed to get data!");
            }
        }
    }
    else
    {
        ZOS_LOG("Gyroscope: Failed to check for new data!");
    }
    ...
    if (ZOS_FAILED(result, zn_adc_direct_sample(adc, &adc_raw)))
        ...
        sprintf((char*)settings.message, MQTT_MESSAGE,
                (int)accl_data.x, (int)accl_data.y, (int)accl_data.z,
                (int)magn_data.x, (int)magn_data.y, (int)magn_data.z,
                (int)gyro_data.x, (int)gyro_data.y, (int)gyro_data.z, rssi_buffer, light_sense_buffer);
        }


    /* Publish the samples to MQTT queue */
